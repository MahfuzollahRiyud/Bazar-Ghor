...
export default function ProductDetail({ product, related }: Props) {
    ...
    return (
        <>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 mb-3">
                            ...
                            <button
                                ...
                                disabled={!inStock}
                                type="button"
                                className={`group/btn relative overflow-hidden flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#2d6a27] py-3.5 px-6 font-bold text-sm text-white shadow-md transition-all hover:bg-[#23531f] active:scale-95 ${
                                    inStock ? 'animate-pulse-subtle' : 'cursor-not-allowed opacity-60'
                                }`}
                            >
                                {/* Psychological Shimmer Light Sweep (Every 3s) */}
                                {inStock && (
                                    <span
                                        aria-hidden="true"
                                ...
                                <Zap size={18} className="fill-current text-yellow-300 relative z-10 transition-transform group-hover/btn:scale-110" />
                                <span className="relative z-10 text-base">{t.buyNow}</span>
                            </button>
                        </div>
                        ...
                        <a
                            ...
                            className="flex w-full items-center justify-center gap-2.5 rounded-xl bg-[#25D366] hover:bg-[#1ebe5d] text-white py-3 px-6 font-bold text-sm shadow-sm transition active:scale-95 mb-4"
                        >
                        ...
                        {/* Psychological Trust & Conversion Badges */}
                        <div className="grid grid-cols-3 gap-2 mb-6 rounded-2xl bg-emerald-50/60 p-3 border border-emerald-100 text-center">
                            <div className="flex flex-col items-center justify-center p-1">
                                <ShieldCheck size={18} className="text-[#2d6a27] mb-1" />
                                <span className="text-[11px] font-bold text-gray-800 leading-tight">
                                    {language === 'bn' ? 'ক্যাশ অন ডেলিভারি' : 'Cash on Delivery'}
                                </span>
                                <span className="text-[10px] text-gray-500 mt-0.5">
                                    {language === 'bn' ? 'দেখে মূল্য দিন' : 'Pay after check'}
                                </span>
                            </div>
                            <div className="flex flex-col items-center justify-center p-1 border-x border-emerald-200/70">
                                <RotateCcw size={18} className="text-[#2d6a27] mb-1" />
                                <span className="text-[11px] font-bold text-gray-800 leading-tight">
                                    {language === 'bn' ? '৭ দিনের রিটার্ন' : '7 Days Return'}
                                </span>
                                <span className="text-[10px] text-gray-500 mt-0.5">
                                    {language === 'bn' ? 'সহজ পলিসি' : 'Easy Exchange'}
                                </span>
                            </div>
                            <div className="flex flex-col items-center justify-center p-1">
                                <Sparkles size={18} className="text-[#2d6a27] mb-1" />
                                <span className="text-[11px] font-bold text-gray-800 leading-tight">
                                    {language === 'bn' ? '১০০% অরিজিনাল' : '100% Original'}
                                </span>
                                <span className="text-[10px] text-gray-500 mt-0.5">
                                    {language === 'bn' ? 'সেরা কোয়ালিটি' : 'Authentic'}
                                </span>
                            </div>
                        </div>
            ...
            <div className="fixed bottom-0 inset-x-0 sm:hidden bg-white/95 backdrop-blur-md p-3 border-t border-gray-200 shadow-2xl z-40 flex items-center justify-between gap-3 animate-in slide-in-from-bottom duration-300">
                <div className="min-w-0">
                    <span className="block text-[11px] text-gray-500 font-medium leading-none mb-1">
                        {language === 'en' ? 'Price' : 'মূল্য'}:
                    </span>
                    <span className="text-lg font-bold text-[#2d6a27] leading-none">
                        ৳{Number(currentPrice).toLocaleString(language === 'bn' ? 'bn-BD' : 'en-US')}
                    </span>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <button
                        ...
                        disabled={!inStock}
                        type="button"
                        className="flex items-center justify-center rounded-xl border border-gray-300 bg-white p-2.5 text-gray-700 active:scale-95 transition"
                        title={t.addToCart}
                    >
                        <ShoppingCart size={18} />
                    </button>
                    ...
                    <button
                        ...
                        disabled={!inStock}
                        type="button"
                        className={`group/btn relative overflow-hidden flex items-center justify-center gap-1.5 rounded-xl bg-[#2d6a27] py-2.5 px-4 font-bold text-sm text-white shadow-md active:scale-95 ${
                            inStock ? 'animate-pulse-subtle' : 'opacity-60 cursor-not-allowed'
                        }`}
                    >
                        {inStock && (
                            <span
                        ...
                        <Zap size={15} className="fill-current text-yellow-300 relative z-10" />
